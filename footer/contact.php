...
<head>
  ...
  <style>
    ...
    .form-group label { font-size: 11px; text-transform: uppercase; letter-spacing: .7px; color: var(--text-dim); }
    .form-group input,
    .form-group textarea,
    .form-group select {
    ...
    .form-group select:focus { border-color: rgba(255,92,26,.5); }
    ...
    .contact-info-item:hover { border-color: rgba(255,92,26,.25); }
    .ci-icon { font-size: 22px; flex-shrink: 0; margin-top: 2px; }
    .ci-label { font-size: 11px; text-transform: uppercase; letter-spacing: .7px; color: var(--text-dim); margin-bottom: 4px; }
...
function submitForm() {
  ...
  if (!fname || !email || !topic || !message) {
    showError('Please fill in First Name, Email, Topic and Message.'); return;
  }
  if (!/\S+@\S+\.\S+/.test(email)) {
